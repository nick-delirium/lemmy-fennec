...
function Comment({ comment, hide }: { comment: CommentNode, hide?: () => void }) {
  const sch = useColorScheme()
  const { colors } = useTheme()

  const commentDate = React.useMemo(() => makeDateString(comment.comment.published), [])
  ...
  const upvoteComment = React.useCallback(() => {
    ...
    void apiClient.commentsStore.rateComment(
      comment.comment.id, apiClient.loginDetails, Score.Upvote)
  }, [])
  const downvoteComment = React.useCallback(() => {
    Vibration.vibrate(50)
    void apiClient.commentsStore.rateComment(
      comment.comment.id, apiClient.loginDetails, Score.Downvote)
  }, [])

  return (
    <View style={{ ...styles.container, borderBottomColor: colors.card }}>
      <View style={styles.topRow}>
        <Text style={styles.author}>u/{comment.creator.display_name || comment.creator.name}</Text>
      ...
      <View style={styles.bottomRow}>
        <View style={styles.infoPiece}>
          <Icon accessibilityLabel={"total rating (upvote percent)"} name={"chevrons-up"} size={24} />
          <Text>
            {comment.counts.score} ({Math.ceil(
            (comment.counts.upvotes / (comment.counts.upvotes + comment.counts.downvotes)) * 100)}%)
          </Text>
        </View>
        {comment.children.length > 0 ? (
          <View style={styles.infoPiece}>
            <Icon accessibilityLabel={"sub comments amount"} name={"message-square"} size={24} />
            <Text>
