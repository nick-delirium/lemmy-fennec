...
const sortTypes = Object.values(SortTypeMap).map(type => ({ label: splitCamelCase(type), value: type }))
const listingTypes = Object.values(ListingTypeMap).map(type => ({ label: splitCamelCase(type), value: type }))
...
function FloatingMenu() {
  ...
  return (
    <View style={styles.container}>
      ...
      {isListingOpen ? <ListingMenu colors={colors} closeSelf={closeAll} /> : null}
      {isOpen ? (
...
function ListingMenu({ colors, closeSelf }: { colors: Theme['colors'], closeSelf: () => void }) {
  const setListing = (listing: typeof ListingTypeMap[keyof typeof ListingTypeMap]) => {
  ...
  return (
    <View style={{ ...styles.menu, backgroundColor: colors.border }}>
      {listingTypes.map((type) => (
        <TouchableOpacity key={type.value} onPress={() => setListing(type.value)}>
          <Text style={{ fontWeight: '500' }}>{type.label}</Text>
        </TouchableOpacity>
      ))}
    </View>
